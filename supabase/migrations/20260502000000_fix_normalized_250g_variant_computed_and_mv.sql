-- Migration: Fix normalized 250g pricing semantics
-- Description: Narrow valid_for_best_value on variant_computed to actual ~250g packs (240–260g).
--              coffee_directory_mv prefers that price but falls back to best derived per-250g
--              when no 250g pack exists (COALESCE).

-- ============================================================================
-- 1. Update variant_computed (was not versioned; align with live definition)
-- ============================================================================

CREATE OR REPLACE VIEW public.variant_computed AS
SELECT
  vlp.variant_id,
  vlp.coffee_id,
  vlp.in_stock,
  ...
  vlp.grind,
  vlp.price_one_time,
  vlp.compare_at_price,
  vlp.currency,
  vlp.scraped_at_latest,
  CASE
    WHEN vlp.price_one_time IS NULL OR vlp.weight_g IS NULL OR vlp.weight_g <= 0 THEN NULL::numeric
    ELSE round(
      (vlp.price_one_time / greatest(vlp.pack_count, 1)::numeric)
        * (250.0 / vlp.weight_g::numeric),
      2
    )
  END AS normalized_250g,
  (vlp.weight_g IS NOT NULL AND vlp.weight_g BETWEEN 240 AND 260) AS valid_for_best_value
FROM public.variant_latest_price vlp;

COMMENT ON VIEW public.variant_computed IS
  'Per-variant pricing; normalized_250g scales to 250g; valid_for_best_value is true only for ~250g packs.';

-- ============================================================================
-- 2. Recreate coffee_directory_mv (prefers 250g pack, else best derived)
-- ============================================================================
...
-- ============================================================================
-- 3. Recreate refresh helper (CASCADE dropped it)
-- ============================================================================
...
-- Initial load is populated by CREATE MATERIALIZED VIEW; concurrent refresh is for operators/cron.
