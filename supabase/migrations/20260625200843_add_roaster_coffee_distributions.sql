-- Migration: Add roast_distribution and process_distribution to get_roaster_detail
-- Description: The roaster detail page renders a breakdown of the roaster's
--   coffees by roast level and process. These distributions must cover ALL of
--   the roaster's public coffees, not just the limited coffees[] array the RPC
--   returns, so they are aggregated server-side here (cheap GROUP BY over
--   coffee_directory_mv on the indexed (roaster_id, status)). Keeps the page on
--   a single round-trip. Drop + recreate to avoid duplicate function definitions.
...
COMMENT ON FUNCTION public.get_roaster_detail(text, int) IS
  'Single-call jsonb assembly of RoasterDetail. coffees[] from coffee_directory_mv (limited, public statuses, name asc). coffee_count/active_coffee_count/avg_coffee_rating computed live over ALL coffees from the coffees table. roast_distribution/process_distribution aggregate ALL public (active+seasonal) coffees by roast level and process (value+count, nulls excluded). Returns NULL when not found.';
