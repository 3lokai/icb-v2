-- Non-coffee products 404 on the detail page.
--
-- 20260718095036 removed combo packs, subscriptions and wholesale SKUs from
-- coffee_directory_mv, but get_coffee_detail reads `FROM coffees c LEFT JOIN
-- coffee_directory_mv mv`, so those pages still resolved — just with every
-- MV-derived field (price, stock, rating) null. Half-populated pages are worse
-- than absent ones, so the RPC now excludes them outright.
--
-- Returning NULL is all that is needed: fetchCoffeeByRoasterAndSlug already
-- maps a null RPC result to null, and the page calls notFound() on that, so
-- these URLs become genuine 404s.
--
-- Body copied verbatim from 20260709121809_fix_get_coffee_detail_use_mv.sql;
-- the only change is the added is_coffee condition, marked "-- ADDED".
