-- Exclude non-coffee products from the coffee directory.
--
-- coffees.is_coffee marks rows that are not a single coffee: combo packs,
-- subscription boxes, wholesale SKUs, merch, and one chocolate bar. Nothing has
-- ever read that flag — not the MV, not any query, not any RPC — so setting it
-- to false had no effect and 21 active non-coffees were listed in the directory,
-- returned by search, and counted in facets.
--
-- Body copied from 20260718092346_add_display_name_to_coffee_directory_mv.sql.
-- The only change is the WHERE clause on the outer SELECT, marked "-- ADDED".
--
-- IS NOT FALSE rather than = true on purpose: ingestion may insert rows before
-- classification sets the flag, and `= true` would make every newly scraped
-- coffee invisible until then. Unclassified (null) rows stay visible.
--
-- search_directory() is NOT recreated here — DROP ... CASCADE does not drop
-- functions, and it already reads from this MV, so it inherits the filter.
...
-- ADDED: keep the directory to actual coffees.
WHERE c.is_coffee IS NOT FALSE;
