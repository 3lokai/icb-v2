-- Migration: Add slug/key fields to get_coffee_filter_meta facets
-- Description: The filter UI uses roasters[].slug, regions[].slug and
--   estates[].key as filter values (CoffeeFilterSidebar/CoffeeFacetedFilterBar),
--   matching the CoffeeFilterMeta type — but the RPC never returned them.
--   Adds them so the RPC can replace the 11-query fetchCoffeeFilterMeta impl.
--   Signature unchanged; body otherwise identical to 20260222100001.

DROP FUNCTION IF EXISTS public.get_coffee_filter_meta;
...
CREATE OR REPLACE FUNCTION public.get_coffee_filter_meta(
  ...
  p_works_with_milk BOOLEAN DEFAULT NULL
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH filtered_coffees AS (
    ...
    WHERE 1=1
      ...
      AND (p_works_with_milk IS NULL OR c.works_with_milk = p_works_with_milk)
  )
  ...
  SELECT json_build_object(
      ...
      FROM (
        ...
        WHERE c.coffee_id = ANY(
          ...
          WHERE c2.coffee_id = ANY(base_filtered_coffee_ids)
            ...
            AND (p_works_with_milk IS NULL OR c2.works_with_milk = p_works_with_milk)
        )
    ...
    'regions', (
      SELECT json_agg(json_build_object('id', sub.id, 'slug', sub.slug, 'label', sub.display_name, 'count', sub.coffee_count) ORDER BY sub.coffee_count DESC, sub.display_name ASC)
      FROM (
        SELECT r.id, COALESCE(cn.slug, r.id::TEXT) AS slug, COALESCE(r.display_name, r.id::TEXT) AS display_name, COUNT(DISTINCT cr.coffee_id) AS coffee_count
        FROM coffee_regions cr
        JOIN regions r ON r.id = cr.region_id
        LEFT JOIN canon_regions cn ON cn.id = r.canon_region_id
        ...
        GROUP BY r.id, r.display_name, cn.slug
        HAVING COUNT(DISTINCT cr.coffee_id) > 0
      ) sub
    ),
    'estates', (
      SELECT json_agg(json_build_object('id', sub.id, 'key', sub.estate_key, 'label', sub.name, 'count', sub.coffee_count) ORDER BY sub.coffee_count DESC, sub.name ASC)
      FROM (
        SELECT e.id, COALESCE(e.estate_key, e.id::TEXT) AS estate_key, e.name, COUNT(DISTINCT ce.coffee_id) AS coffee_count
        ...
        GROUP BY e.id, e.estate_key, e.name
    ...
    'roasters', (
      SELECT json_agg(json_build_object('id', sub.id, 'slug', sub.slug, 'label', sub.name, 'count', sub.coffee_count) ORDER BY sub.coffee_count DESC, sub.name ASC)
      FROM (
        SELECT r.id, r.slug, r.name, COUNT(DISTINCT c.coffee_id) AS coffee_count
        FROM coffee_directory_mv c
...
GRANT EXECUTE ON FUNCTION public.get_coffee_filter_meta
  TO anon, authenticated, service_role;

COMMENT ON FUNCTION public.get_coffee_filter_meta IS
  'Returns filter metadata with counts for coffee directory filtering. Uses coffee_directory_mv. Includes p_bean_species and p_works_with_milk filters. Facets carry the UI filter values: roasters[].slug, regions[].slug (canon_regions), estates[].key (estate_key).';
