-- get_coffee_detail: return display_name.
--
-- The RPC's jsonb had no display_name, so CoffeeDetail arrived without it and
-- getCoffeeDisplayName() silently fell back to cleaning `name` at request time.
-- That produced clean-looking titles, which hid the actual bug: the fallback
-- cannot strip a trailing roaster name, because the RPC nests the roaster as
-- `roaster.name` rather than the `roaster_name` / `roasters` shapes the helper
-- reads. Result:
--
--   card / directory :  "Coorg Naturals"
--   detail <title>   :  "Coorg Naturals - Red Sirocco by Red Sirocco"
--
-- Returning the backfilled column fixes the mismatch at the source and drops
-- the per-request cleaning work on every coffee page.
--
-- Body copied verbatim from 20260718095450_exclude_non_coffees_from_coffee_detail.sql;
-- the only change is the added jsonb key, marked "-- ADDED".
...
AS $$
  SELECT (
    SELECT jsonb_build_object(
      ...
      'slug',            c.slug,
      'name',            c.name,
      'display_name',    c.display_name,   -- ADDED
    ...
    WHERE r0.slug = p_roaster_slug AND c.slug = p_coffee_slug
      -- ADDED: combo packs / subscriptions / wholesale are not coffees; 404 them.
      AND c.is_coffee IS NOT FALSE
    LIMIT 1
  );
$$;
