...
-- Created: 2026-01-28
-- Description: Updates get_similar_coffees to include both 'active' and 'seasonal' status coffees
--              and adds proper DROP statement for clean re-runs
...
begin
  ...
  -- Exclude current coffee, include active and seasonal status, prefer coffees with images
  select jsonb_agg(
    jsonb_build_object(
      'coffee_id', coffee_id,
      'slug', slug,
  ...
  where coffee_id != p_coffee_id
    ...
    and (
      ...
      or (p_roast_level is not null and roast_level = p_roast_level::roast_level_enum)
      or (p_process is not null and process = p_process::process_enum)
...
comment on function get_similar_coffees is 
  'Finds similar coffees based on flavor overlap, roast level, process, and origin.
   ...
   Includes both active and seasonal status coffees.';
