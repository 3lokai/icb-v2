-- Expose coffees.display_name through coffee_directory_mv.
--
-- The MV powers the coffee directory, and every listing/card render reads from
-- it, so display_name has to be a column here — there is no way to reach it
-- from the app layer otherwise. A matview cannot be ALTERed to add a column,
-- so it is dropped and recreated.
--
-- The body below is the definition from 20260629120000_add_postgres_search.sql
-- copied verbatim. Only three things differ, each marked "-- ADDED":
--   1. c.display_name in the SELECT list
--   2. display_name folded into search_vector (so search matches the cleaned
--      name too — "Maillard's Dream" now matches a row stored as
--      "Maillard&#8217;s Dream")
--   3. a trigram index on display_name, mirroring the one on name
--
-- NOT recreated here, deliberately: refresh_coffee_directory_mv(). DROP ...
-- CASCADE does not drop functions, and that function carries an IndexNow
-- notification body that previous migrations warn must not be regressed.
-- Leaving it untouched is safer than re-emitting it.
...
SELECT
  ...
  c.display_name,                                        -- ADDED
  -- ADDED: display_name folded in so search matches the cleaned name too.
  to_tsvector('simple',
    coalesce(c.name, '') || ' ' ||
...
-- ADDED: mirrors idx_coffee_directory_mv_name_trgm so substring search works
-- against the cleaned name as well.
CREATE INDEX idx_coffee_directory_mv_display_name_trgm
  ON coffee_directory_mv USING gin (display_name gin_trgm_ops);

-- ============================================================================
-- search_directory: return the cleaned name as the result title.
--
-- Search results are a render path, so titles must not show raw scraped names.
-- Definition copied from 20260701140000_search_directory_guards.sql; the only
-- changes are in the COFFEE branch, marked "-- CHANGED":
--   - 'title' and the ILIKE/similarity ranking use coalesce(display_name, name)
--   - the WHERE clause additionally matches display_name, so typing the cleaned
--     form finds a row whose raw name still holds entities
-- Guards (qlen >= 2, lim clamp) and the roaster branch are unchanged.
--
-- The function is NOT dropped by the CASCADE above (Postgres does not track
-- function->matview dependencies), so this is a straight redefinition.
-- ============================================================================
  ...
  SELECT item FROM (
    ...
    SELECT
      jsonb_strip_nulls(jsonb_build_object(
        'id', m.coffee_id,
        'type', 'coffee',
        'title', coalesce(m.display_name, m.name),   -- CHANGED
      ...
      -- CHANGED: rank against the cleaned name users actually see.
      (coalesce(m.display_name, m.name) ILIKE p.prefix_pat)::int * 2
      + (coalesce(m.display_name, m.name) ILIKE p.like_pat)::int
      + ts_rank(m.search_vector, p.tsq)
      + similarity(coalesce(m.display_name, m.name), p.qq) AS rank
    ...
    WHERE p.qlen >= 2                              -- guard: blank/too-short q → no rows
      ...
      AND (m.name ILIKE p.like_pat
           OR coalesce(m.display_name, '') ILIKE p.like_pat   -- CHANGED
           OR m.search_vector @@ p.tsq
           OR m.name % p.qq
           OR coalesce(m.display_name, '') % p.qq)            -- CHANGED
...
COMMENT ON FUNCTION search_directory(text, int) IS
  ...
  'Coffee titles use display_name (cleaned) falling back to name. '
