...
export function RegionSpotlight({ value }: RegionSpotlightProps) {
  ...
  // CTA as a visual affordance, so there is no nested anchor. With no region to
  // link to there is no click target, so drop the hover affordance entirely.
  const href =
    value.link ||
    (regionId ? `/coffees?regions=${encodeURIComponent(regionId)}` : null);
          ...
          {href && (
            <span
              className={cn(
                ...
                "rounded-xl shadow-md transition-all group-hover:translate-y-[-2px] group-hover:shadow-lg bg-emerald-600"
              )}
            >
              Explore Region{" "}
              <Icon
  ...
  return (
    <motion.div
      ...
      className={cn(
        "not-prose my-12 overflow-hidden rounded-2xl border border-border/40 bg-card shadow-lg",
        href && "group transition-all hover:shadow-2xl hover:border-border/80"
      )}
    >
      {href ? (
        <Link
          href={href}
          aria-label={`${name} — explore coffees from this region`}
