...
export async function subscribeToNewsletter(formData: FormData) {
  ...
  try {
    ...
    // Newsletter list membership (fire and forget) — anon and logged-in alike.
    void subscribeToNewsletterList(email, {
      externalId: user?.id,
      fullName: userName,
    });
...
/**
 * Add a newly registered user to the newsletter list.
 * Called after successful email/password signup.
 */
export async function syncUserToNewsletterList(userId: string): Promise<void> {
  try {
    const supabase = await createServiceRoleClient();

    const { data: profile } = await supabase
      .from("user_profiles")
      .select("email, full_name, newsletter_subscribed")
      .eq("id", userId)
      .single();

    if (!profile || !profile.email) {
      console.log("[Notifuse] No profile or email found for user:", userId);
      return;
    }

    // Only sync if newsletter_subscribed is true (default)
    if (!(profile.newsletter_subscribed ?? true)) {
      console.log("[Notifuse] User opted out of newsletter:", userId);
      return;
    }

    await subscribeToNewsletterList(profile.email, {
      externalId: userId,
      fullName: profile.full_name,
    });
  } catch (error) {
    console.error("[Notifuse] Error syncing user to newsletter list:", error);
