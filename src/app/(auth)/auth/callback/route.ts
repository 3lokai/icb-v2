...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    if (userCreatedAt > fiveMinutesAgo) {
      ...
      // the identity provider, not an ICB page. Query params are dropped — the
      ...
      // Add to the newsletter list if newsletter_subscribed is true (default)
      if (user.email && (profile?.newsletter_subscribed ?? true)) {
        void subscribeToNewsletterList(user.email, {
          externalId: user.id,
          fullName: name,
        });
      }
